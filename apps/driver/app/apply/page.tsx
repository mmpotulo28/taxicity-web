        ...
        <div className="flex justify-between mb-8 px-4">
          {steps.map((s, index) => (
            <div key={s.id} className="flex flex-col items-center relative z-10">
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center mb-2 transition-colors ${step >= s.id ? "bg-primary text-white" : "bg-default-200 text-default-500"
                  }`}>
                <Icon icon={s.icon} width={20} />
              </div>
              <span
                className={`text-xs font-medium ${step >= s.id ? "text-primary" : "text-default-500"
                  }`}>
                {s.title}
              </span>
              {index < steps.length - 1 && (
                <div
                  className={`absolute top-5 left-1/2 w-full h-[2px] -z-10 ${step > s.id ? "bg-primary" : "bg-default-200"
                    }`}
              ...
              <motion.div
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <Card
                          isPressable
                          className="w-full h-32 border-2 border-dashed border-default-300 bg-transparent hover:bg-default-100 flex items-center justify-center"
                          onPress={() => document.getElementById("licenseFront")?.click()}>
                          {formData.licenseImageFront ? (
                            <img
                              src={formData.licenseImageFront}
                              alt="Front"
                            ...
                            <div className="flex flex-col items-center">
                              <Icon
                                icon="lucide:camera"
                                className="w-6 h-6 mb-1 text-default-500"
                              />
                              <span className="text-xs text-default-500">
                                {uploading === "licenseImageFront"
                                  ? "Uploading..."
                                  : "Upload Front"}
                              </span>
                            </div>
                      ...
                      <div>
                        ...
                        <Card
                          isPressable
                          className="w-full h-32 border-2 border-dashed border-default-300 bg-transparent hover:bg-default-100 flex items-center justify-center"
                          onPress={() => document.getElementById("licenseBack")?.click()}>
                          {formData.licenseImageBack ? (
                            <img
                              src={formData.licenseImageBack}
                              alt="Back"
                              className="w-full h-full object-cover rounded-lg"
                            />
                          ) : (
                ...
                {step === 3 && (
                  <div className="space-y-6">
                    ...
                    <p className="text-small text-default-500">
                      Select the primary route you will be operating on. You must provide a
                      valid operating permit for this route in the next step.
                    </p>
                    <Select
                      label="Select Route"
                ...
                {step === 4 && (
                  <div className="space-y-6">
                    ...
                    <Card
                      isPressable
                      className={`border-2 border-dashed p-4 w-full ${formData.registrationDoc
                        ? "border-success bg-success-50"
                        : "border-default-300 hover:bg-default-100"
                        }`}
                      onPress={() => document.getElementById("regDoc")?.click()}>
                      <CardBody className="flex flex-row items-center gap-4">
                        ...
                        <div className="text-left">
                          <p
                            className={`text-sm font-medium ${formData.registrationDoc ? "text-success-700" : ""
                              }`}>
                            {formData.registrationDoc
                    ...
                    <Card
                      isPressable
                      className={`border-2 border-dashed p-4 w-full ${formData.insuranceDoc
                        ? "border-success bg-success-50"
                        : "border-default-300 hover:bg-default-100"
                        }`}
                      onPress={() => document.getElementById("insDoc")?.click()}>
                      <CardBody className="flex flex-row items-center gap-4">
                        ...
                        <div className="text-left">
                          <p
                            className={`text-sm font-medium ${formData.insuranceDoc ? "text-success-700" : ""
                              }`}>
                            {formData.insuranceDoc
                    ...
                    <Card
                      isPressable
                      className={`border-2 border-dashed p-4 w-full ${formData.permitDoc
                        ? "border-success bg-success-50"
                        : "border-default-300 hover:bg-default-100"
                        }`}
                      onPress={() => document.getElementById("permitDoc")?.click()}>
                      <CardBody className="flex flex-row items-center gap-4">
                        {formData.permitDoc ? (
                          <Icon
                            icon="lucide:check-circle"
                        ...
                        <div className="text-left">
                          <p
                            className={`text-sm font-medium ${formData.permitDoc ? "text-success-700" : ""
                              }`}>
                            {formData.permitDoc
                              ? "Operating Permit Uploaded"
                              : "Upload Operating Permit"}
                          </p>
                          {!formData.permitDoc && (
                            <p className="text-xs text-default-400">
                              {uploading === "permitDoc"
