    ...
    <div className="p-4 pb-24 space-y-4">
      ...
      {notifications.length === 0 ? (
        ...
        notifications.map((notification) => (
          <Card
            key={notification.id}
            className={`w-full ${notification.read ? 'opacity-60' : 'border-l-4 border-primary'}`}
          >
            <CardBody className="flex flex-row gap-4 items-start">
              ...
              <div className="flex-1">
                <div className="flex justify-between items-start">
                  <h3 className={`font-semibold ${notification.read ? 'text-default-600' : 'text-default-900'}`}>
                    {notification.title}
                  </h3>
                ...
                {!notification.read && (
                  <Button
                    ...
                    variant="light"
                    color="primary"
                    className="mt-2 px-0 h-8 min-w-0"
