...
type DriverTaxi = Driver["taxis"][number];

export default function DriverVehiclePage() {
	const { driver, isLoading, refreshDriver } = useDriver();
	const [showRegistration, setShowRegistration] = useState(false);
	const [selectedTaxiForRoute, setSelectedTaxiForRoute] = useState<DriverTaxi | null>(null);

	const handleEditVehicle = (_taxi: DriverTaxi) => {
		addToast({
			title: "Coming Soon",
	...
	const handleManageRoute = (taxi: DriverTaxi) => {
		...
		return (
			<div className='p-4 pb-24'>
				<div className='flex items-center gap-2 mb-4'>
					{hasVehicles && (
						<Button isIconOnly variant='light' onPress={() => setShowRegistration(false)}>
	...
	return (
		<div className='p-4 pb-24'>
			<div className='flex justify-between items-center mb-4'>
				...
				<Button color='primary' onPress={() => setShowRegistration(true)} startContent={<Icon icon='lucide:plus' />}>
					Add Vehicle
				</Button>
			</div>
			...
			<div className='flex flex-col gap-4'>
				{driver.taxis.map((taxi) => (
					<VehicleCard key={taxi.id} taxi={taxi} onEdit={handleEditVehicle} onManageRoute={handleManageRoute} />
				))}
			</div>
			...
			<RouteManagementModal
				...
				currentRouteId={selectedTaxiForRoute?.routes?.find((r) => r.isActive)?.route.id}
				onSuccess={() => {
					refreshDriver();
