	...
	return (
		<Modal isOpen={isOpen} onClose={onClose}>
			<ModalContent>
				...
				<ModalBody>
					<p className='text-small text-default-500 mb-4'>To change your route, you must select a new route and upload a valid operating permit for that route.</p>
					...
					<div className='mt-4'>
						...
						<div className={`border-2 border-dashed rounded-lg p-4 text-center cursor-pointer transition-colors ${permitFile ? "border-success bg-success-50" : "border-default-300 hover:bg-default-100"}`} onClick={() => document.getElementById("modalPermitDoc")?.click()}>
							{permitFile ? (
								<>
									<Icon icon='lucide:check-circle' className='w-6 h-6 mx-auto mb-1 text-success' />
									<p className='text-xs font-medium text-success-700'>{permitFile.name}</p>
								</>
							) : (
								<>
									<Icon icon='lucide:upload' className='w-6 h-6 mx-auto mb-1 text-default-500' />
				...
				<ModalFooter>
					...
					<Button color='primary' onPress={handleSubmit} isLoading={loading} isDisabled={!selectedRoute || !permitFile}>
