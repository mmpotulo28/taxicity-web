...
export default function ActiveShiftScreen() {
	...
	if (isError) {
		return <ErrorState title='Unable to load active shift' message={error?.message ?? "Unknown shift load error"} onRetry={refresh} />;
	}
	...
	const onEndShift = async () => {
		const endedShift = await endShift.mutateAsync(shift.id);
		router.replace({
			pathname: "/(shift)/end-summary",
			params: {
				shiftId: endedShift.id,
				shiftStatus: endedShift.status,
				routeName: endedShift.route.name,
				taxiPlate: endedShift.taxi.licensePlate,
			},
		});
	};
