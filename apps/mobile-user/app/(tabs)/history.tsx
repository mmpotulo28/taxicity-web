...
import { useTripHistoryInfinite } from "../../src/features/ride/hooks/use-trip-history";

export default function HistoryTabScreen() {
	const [searchQuery, setSearchQuery] = useState("");
	const { infiniteTripsQuery, trips } = useTripHistoryInfinite();
	const pagination = infiniteTripsQuery.data?.pages.at(-1)?.pagination;
	...
	return (
		<View className='flex-1 bg-black px-4 pt-12'>
			...
			<Pressable className='mb-3 self-start rounded-md border border-neutral-700 px-4 py-2' onPress={() => infiniteTripsQuery.refetch()}>
				<Text className='text-white'>{infiniteTripsQuery.isFetching ? "Refreshing..." : "Refresh"}</Text>
			</Pressable>
			<TextInput
				value={searchQuery}
			...
			{infiniteTripsQuery.isLoading ? (
				...
				<View className='rounded-md border border-red-800 bg-red-950/40 p-4'>
					...
					<Pressable className='mt-3 rounded-md border border-red-700 px-4 py-2' onPress={() => infiniteTripsQuery.refetch()}>
				...
				<ScrollView className='flex-1'>
					...
					{filteredTrips.length > 0 && pagination ? (
						<Text className='mb-3 mt-1 text-xs text-neutral-500'>
							Page {pagination.page} of {pagination.pages} • {pagination.total} total trips
						</Text>
					) : null}
					{infiniteTripsQuery.hasNextPage ? (
						<Pressable
							className='mb-6 rounded-md border border-neutral-700 px-4 py-3'
							onPress={() => infiniteTripsQuery.fetchNextPage()}
							disabled={infiniteTripsQuery.isFetchingNextPage}
						>
							<Text className='text-center text-white'>
								{infiniteTripsQuery.isFetchingNextPage ? "Loading more..." : "Load more"}
							</Text>
						</Pressable>
					) : filteredTrips.length > 0 ? (
						<Text className='mb-6 text-center text-xs text-neutral-500'>You have reached the end.</Text>
					) : null}
				</ScrollView>
			)}
		</View>
