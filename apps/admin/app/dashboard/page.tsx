		...
		<div className='space-y-6'>
			<div className='flex justify-between items-center'>
				...
				<div className='flex gap-2'>
					...
					<Dropdown>
						<DropdownTrigger>
							<Button color='primary' endContent={<Icon icon='lucide:chevron-down' />}>
								Export
							</Button>
						</DropdownTrigger>
						<DropdownMenu aria-label='Export options'>
							...
							<DropdownItem key='excel' startContent={<Icon icon='lucide:file-spreadsheet' />}>
								Export as Excel
							</DropdownItem>
			...
			<div className='flex justify-end'>
				<Tabs aria-label='Time Range' color='primary' selectedKey={timeRange} size='sm' onSelectionChange={(key) => setTimeRange(key as string)}>
					...
					<CardBody className='p-4'>
						{isLoading ? (
							<div className='h-64 bg-default-100 rounded-lg animate-pulse flex items-center justify-center'>
								<Icon className='text-4xl text-default-300' icon='lucide:loader-2' />
							</div>
						) : (
							<div className='h-64'>
			...
			<Card>
				<CardHeader className='pb-0 flex justify-between'>
					...
					<Button color='primary' size='sm' variant='flat' onPress={() => router.push("/secure/dashboard/trips")}>
						View All
					</Button>
				</CardHeader>
				<CardBody className='p-4'>
					{isTripsLoading ? (
						<div className='space-y-4'>
							{Array.from({ length: 5 }).map((_, i) => (
								<div key={i} className='h-12 bg-default-100 rounded-md animate-pulse' />
							))}
						</div>
					) : (
						<table className='min-w-full'>
							<thead>
								<tr className='border-b border-divider'>
									<th className='text-left py-3 text-default-500 text-sm font-medium'>Event</th>
									<th className='text-left py-3 text-default-500 text-sm font-medium'>User</th>
									<th className='text-left py-3 text-default-500 text-sm font-medium'>Route</th>
									<th className='text-left py-3 text-default-500 text-sm font-medium'>Time</th>
									<th className='text-left py-3 text-default-500 text-sm font-medium'>Status</th>
								</tr>
							</thead>
							<tbody>
								{recentTrips?.slice(0, 5).map((trip) => (
									<tr key={trip.id} className='border-b border-divider hover:bg-default-50 transition-colors'>
										<td className='py-3'>Trip {trip.status.charAt(0).toUpperCase() + trip.status.slice(1).toLowerCase()}</td>
										<td className='py-3'>Customer #{trip.userId.slice(0, 6)}...</td>
										...
										<td className='py-3'>
											<span className={`px-2 py-1 rounded-full text-xs ${trip.status === "COMPLETED" ? "bg-success-100 text-success-600" : trip.status === "CANCELLED" ? "bg-danger-100 text-danger-600" : "bg-warning-100 text-warning-600"}`}>{trip.status}</span>
										</td>
									</tr>
								))}
							</tbody>
					...
					<div className='mt-4 flex justify-between items-center'>
						<p className='text-sm text-default-500'>
							Showing <span className='font-medium'>{Math.min(5, recentTrips?.length || 0)}</span> of <span className='font-medium'>{recentTrips?.length || 0}</span> activities
						</p>
			...
			<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
				<Card isPressable className='shadow-sm' onPress={() => router.push("/secure/dashboard/drivers")}>
					<CardBody className='p-4'>
						<div className='flex items-center gap-3'>
							...
							<div>
								...
								<p className='text-xs text-default-500'>Register and approve new drivers</p>
							</div>
						</div>
					</CardBody>
				</Card>

				<Card isPressable className='shadow-sm' onPress={() => router.push("/secure/dashboard/taxis")}>
					<CardBody className='p-4'>
						<div className='flex items-center gap-3'>
							...
							<div>
								...
								<p className='text-xs text-default-500'>Add new taxis to the system</p>
							</div>
						</div>
					</CardBody>
				</Card>

				<Card isPressable className='shadow-sm' onPress={() => router.push("/secure/dashboard/routes")}>
					<CardBody className='p-4'>
						<div className='flex items-center gap-3'>
							...
							<div>
								...
								<p className='text-xs text-default-500'>Create and update taxi routes</p>
							</div>
						</div>
					</CardBody>
				</Card>

				<Card isPressable className='shadow-sm' onPress={() => router.push("/secure/dashboard/reports")}>
					<CardBody className='p-4'>
						<div className='flex items-center gap-3'>
							...
							<div>
								...
								<p className='text-xs text-default-500'>Create detailed analytics reports</p>
							</div>
						</div>
					</CardBody>
				</Card>
