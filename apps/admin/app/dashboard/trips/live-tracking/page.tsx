	...
	const fetchActiveTrips = useCallback(async () => {
		try {
			...
			if (tripId) {
				...
				const avgLat =
					trips.reduce((sum: number, t: ActiveTrip) => {
			...
			<header className='bg-background border-b border-divider p-4'>
				<div className='max-w-7xl mx-auto flex items-center justify-between'>
					...
					<div className='flex gap-2'>
						<Button size='sm' variant='flat' startContent={<Icon icon='lucide:refresh-cw' />} onPress={fetchActiveTrips}>
							Refresh
						</Button>
			...
			<div className='flex-1 flex overflow-hidden'>
					...
					<div className='p-4 space-y-3'>
						...
						{!loading &&
							...
							activeTrips.map((trip) => (
								<Card key={trip.id} isPressable isHoverable className={`cursor-pointer transition-all ${selectedTrip?.id === trip.id ? "ring-2 ring-primary" : ""}`} onPress={() => handleTripSelect(trip)}>
									<CardBody className='p-3'>
										<div className='flex items-start justify-between mb-2'>
											<Chip color={getTripStatusColor(trip.status)} size='sm' variant='flat'>
												{trip.status.replace("_", " ")}
											</Chip>
											<span className='text-xs text-default-500'>{new Date(trip.requestTime).toLocaleTimeString()}</span>
										</div>
										...
										<div className='space-y-2'>
											...
											<div className='flex items-start gap-2'>
												...
												<div className='flex-1 min-w-0'>
													...
													<p className='text-sm font-medium truncate'>{trip.dropoffAddress}</p>
												</div>
											</div>
											...
											{trip.vehicleTrip && (
												<div className='flex items-center gap-2 pt-2 border-t border-divider'>
													...
													<span className='text-sm'>{trip.vehicleTrip.driver.fullName || `${trip.vehicleTrip.driver.firstName} ${trip.vehicleTrip.driver.lastName}`}</span>
										...
										<div className='flex items-center justify-between mt-3 pt-2 border-t border-divider'>
											<span className='text-sm font-semibold text-success'>R{Number(trip.fare).toFixed(2)}</span>
											<Button as={Link} href={`/dashboard/trips/${trip.id}`} size='sm' variant='light' color='primary' endContent={<Icon icon='lucide:arrow-right' />}>
												Details
											</Button>
										</div>
									</CardBody>
				...
				<div className='flex-1 relative'>
							...
							{activeTrips.map((trip) => (
								<div key={trip.id}>
										...
										<AdvancedMarker
											...
											<div className='relative'>
												<svg
													...
													style={{
														transform: trip.currentLocation.heading ? `rotate(${trip.currentLocation.heading}deg)` : undefined,
													}}>
													<path d='M18.92 6.01C18.72 5.42 18.16 5 17.5 5h-11c-.66 0-1.21.42-1.42 1.01L3 12v8c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-1h12v1c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-8l-2.08-5.99zM6.5 16c-.83 0-1.5-.67-1.5-1.5S5.67 13 6.5 13s1.5.67 1.5 1.5S7.33 16 6.5 16zm11 0c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zM5 11l1.5-4.5h11L19 11H5z' />
												</svg>
												{selectedTrip?.id === trip.id && <div className='absolute -top-2 -right-2 w-4 h-4 bg-primary rounded-full animate-ping' />}
												{/* Speed indicator */}
												{trip.currentLocation.speed !== undefined && <div className='absolute -bottom-6 left-1/2 transform -translate-x-1/2 bg-primary text-white text-xs px-2 py-0.5 rounded-full whitespace-nowrap'>{Math.round(trip.currentLocation.speed)} km/h</div>}
											</div>
										</AdvancedMarker>
									)}
									...
									<AdvancedMarker
										...
										<div className='relative'>
											<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='#22c55e' width='28' height='28' className='drop-shadow-lg' style={{ opacity: trip.currentLocation ? 0.6 : 1 }}>
									...
									{/* Dropoff Marker */}
									<AdvancedMarker position={{ lat: trip.dropoffLat, lng: trip.dropoffLng }} title={`Dropoff: ${trip.dropoffAddress}`}>
										<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='#ef4444' width='28' height='28' className='drop-shadow-lg' style={{ opacity: trip.currentLocation ? 0.6 : 1 }}>
											<path d='M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5a2.5 2.5 0 0 1 0-5 2.5 2.5 0 0 1 0 5z' />
										</svg>
									</AdvancedMarker>
									...
									{selectedMarker === trip.id && (
										<InfoWindow position={trip.currentLocation ? { lat: trip.currentLocation.lat, lng: trip.currentLocation.lng } : { lat: trip.pickupLat, lng: trip.pickupLng }} onCloseClick={() => setSelectedMarker(null)}>
											<div className='p-2 min-w-[200px]'>
												<p className='font-semibold mb-2'>{trip.route.name}</p>
												<Chip color={getTripStatusColor(trip.status)} size='sm' variant='flat' className='mb-2'>
													{trip.status.replace("_", " ")}
												</Chip>
												{trip.vehicleTrip && (
													<div className='text-sm space-y-1'>
														<p>
															<strong>Driver:</strong> {trip.vehicleTrip.driver.fullName || `${trip.vehicleTrip.driver.firstName} ${trip.vehicleTrip.driver.lastName}`}
														</p>
														<p>
												...
												<Link href={`/dashboard/trips/${trip.id}`}>
													<Button size='sm' color='primary' className='w-full mt-3' endContent={<Icon icon='lucide:arrow-right' />}>
														View Details
													</Button>
												</Link>
											</div>
					...
					<div className='absolute top-4 right-4'>
						<Chip color={socketRef.current?.connected ? "success" : "warning"} variant='flat' startContent={<div className={`w-2 h-2 rounded-full ${socketRef.current?.connected ? "bg-success animate-pulse" : "bg-warning"}`} />}>
...
function LiveTrackingFallback() {
	return (
		<div className='flex items-center justify-center min-h-screen'>
			<Card>
				<CardBody>
					<p className='text-default-500'>Loading live tracking...</p>
				</CardBody>
			</Card>
		</div>
	);
...
export default function LiveTrackingPage() {
	return (
		<Suspense fallback={<LiveTrackingFallback />}>
			<LiveTrackingContent />
		</Suspense>
	);
}
