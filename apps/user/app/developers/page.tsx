	...
	return (
		<div className='container mx-auto px-4 py-8 max-w-6xl'>
			<div className='text-center mb-12'>
				...
				<p className='text-xl text-default-600 max-w-2xl mx-auto'>Everything you need to integrate with the TaxiCity platform. Explore our APIs, documentation, and developer tools.</p>
			</div>
				...
				<Card className='p-2'>
					<CardHeader className='flex items-start gap-3'>
						...
						<div>
							...
							<p className='text-sm text-default-600'>Explore our complete REST API with interactive examples</p>
						</div>
					</CardHeader>
					<CardBody>
					...
					<CardFooter>
						<Button as={Link} href='/api-docs' color='primary' variant='flat' className='w-full' endContent={<ExternalLinkIcon className='h-4 w-4' />}>
							View API Docs
						</Button>
					</CardFooter>
				</Card>
				...
				<Card className='p-2'>
					<CardHeader className='flex items-start gap-3'>
						...
						<div>
							...
							<p className='text-sm text-default-600'>Check the current status and health of our APIs</p>
						</div>
					</CardHeader>
					<CardBody>
						<ul className='space-y-2 text-sm'>
					...
					<CardFooter>
						<Button as={Link} href='/api/user/status' color='success' variant='flat' className='w-full' endContent={<ExternalLinkIcon className='h-4 w-4' />}>
							Check Status
						</Button>
					</CardFooter>
				</Card>
				...
				<Card className='p-2'>
					<CardHeader className='flex items-start gap-3'>
						...
						<div>
							...
							<p className='text-sm text-default-600'>Download or import our OpenAPI 3.0 specification</p>
						</div>
					</CardHeader>
					<CardBody>
						<ul className='space-y-2 text-sm'>
					...
					<CardFooter>
						<Button as={Link} href='/api/user/openapi' color='warning' variant='flat' className='w-full' endContent={<ExternalLinkIcon className='h-4 w-4' />}>
							Get OpenAPI Spec
						</Button>
					</CardFooter>
				</Card>
		...
		const token = await getToken({ template: 'taxiciti_api' });`}</pre>
						...
						<div>
							...
							<div className='bg-default-100 rounded-lg p-4 font-mono text-sm overflow-x-auto'>
								<pre>{`const response = await fetch('/api/user/taxis', {
  headers: {
				...
				<CardBody>
					<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
						{[
							...
							{
								name: "Drivers",
								endpoint: "/api/user/drivers",
								description: "Driver management",
							},
							{
								name: "Taxis",
								endpoint: "/api/user/taxis",
							...
							<div key={api.name} className='border border-default-200 rounded-lg p-4'>
								<h4 className='font-semibold text-primary'>{api.name}</h4>
								<p className='text-sm text-default-600 mb-2'>{api.description}</p>
								<code className='text-xs bg-default-100 px-2 py-1 rounded'>{api.endpoint}</code>
							</div>
						))}
					</div>
				</CardBody>
