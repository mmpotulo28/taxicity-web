...
const TrackRide: React.FC = () => {
	...
	const { isOpen: isCancelConfirmOpen, onOpen: onCancelConfirmOpen, onOpenChange: onCancelConfirmOpenChange } = useDisclosure();
	...
	const currentTaxiLocation = useMemo(
		() =>
			trackedLocation
				? {
				...
				: activeTaxi?.location,
		[trackedLocation, activeTaxi?.location],
	);
	...
	useEffect(() => {
		...
		if (activeTrip.pickupLat && activeTrip.pickupLng && !arrivalNotified) {
			const distance = calculateDistance(currentTaxiLocation.lat, currentTaxiLocation.lng, activeTrip.pickupLat, activeTrip.pickupLng);
			...
			<div className='absolute bottom-0 left-0 right-0 p-4 z-10'>
				<Card className='w-full shadow-lg bg-background/95 backdrop-blur-md border-t border-default-200'>
					<CardBody className='p-4'>
							...
							<div className='flex items-center gap-4 mb-6 bg-default-50 p-3 rounded-xl'>
								...
								<div className='flex-1'>
									...
									<p className='text-xs text-default-500'>
										{activeTrip.vehicle} • {activeTrip.licensePlate}
									</p>
								</div>
						...
						<Button color='danger' variant='light' className='w-full' onPress={onCancelConfirmOpen}>
							Cancel Request
						</Button>
					</CardBody>
				</Card>
		...
		<div className='absolute bottom-0 left-0 right-0 p-4 z-10'>
			<Card className='w-full shadow-lg bg-background/95 backdrop-blur-md border-t-4 border-primary'>
				<CardBody className='p-6 text-center'>
					...
					<Button color='primary' size='lg' className='w-full font-semibold shadow-lg shadow-primary/20' startContent={<Icon icon='lucide:qr-code' />} onPress={onScannerOpen}>
						Scan QR Code
					</Button>
				</CardBody>
			</Card>
			...
			<div className='bg-background rounded-t-2xl shadow-[0_-4px_20px_rgba(0,0,0,0.1)] flex-1 overflow-hidden flex flex-col'>
				...
				<div className='px-4 pb-2 border-b border-default-100'>
					<div className='flex justify-between items-center'>
						...
						<span className='text-xs font-medium text-default-500'>{selectedRoute?.popularLocations?.length || 0} stops</span>
					</div>
				</div>
				...
				<div className='overflow-y-auto p-4 space-y-0 relative'>
					...
					{selectedRoute?.popularLocations?.map((stop, index) => (
						<div key={stop.id} className='relative flex gap-4 pb-6 last:pb-0 group'>
							{/* Dot */}
							<div
								className={`relative z-10 w-3 h-3 mt-1.5 rounded-full border-2 flex-shrink-0
								${index === 0 ? "bg-primary border-primary ring-4 ring-primary/20" : "bg-background border-default-400"}`}
							/>

							<div className='flex-1'>
								<div className='flex justify-between items-start'>
									<p className={`text-sm font-medium ${index === 0 ? "text-primary" : "text-default-700"}`}>{stop.name}</p>
									{index === 0 && (
	...
	return (
		<div className='h-full relative bg-default-100 overflow-hidden'>
			{/* Map Layer */}
			<MapView showTaxis={activeTrip.status !== "in-progress"} showRoute={activeTrip.status === "in-progress"} taxiLocation={currentTaxiLocation} />
			...
			<div className='absolute top-0 left-0 right-0 p-4 z-10 flex justify-between items-start pointer-events-none bg-[rgba(0,0,0,0.9)] rounded-b-xl'>
				<Button isIconOnly size='lg' className='bg-default/80 backdrop-blur-md shadow-sm pointer-events-auto' onPress={() => router.back()}>
					<Icon icon='lucide:arrow-left' />
				</Button>

				{activeTrip.status === "in-progress" && (
					<div className='flex gap-2 pointer-events-auto items-start'>
						<Button isIconOnly size='lg' className='bg-default/80 backdrop-blur-md shadow-sm' onPress={shareRide}>
			...
			<AnimatePresence mode='wait'>
				{(activeTrip.status === "requested" || activeTrip.status === "accepted") && (
					<motion.div key='waiting' initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }}>
						<WaitingView />
					</motion.div>
				)}

				{activeTrip.status === "driver-arrived" && (
					<motion.div key='boarding' initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }}>
						<BoardingView />
					</motion.div>
				)}

				{activeTrip.status === "in-progress" && (
					<motion.div key='tracking' initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }} className='h-full flex flex-col justify-end'>
						<TrackingView />
					</motion.div>
				)}
			</AnimatePresence>
						...
						<>
							<ModalHeader className='flex flex-col gap-1'>
								...
								<div className='flex items-center gap-2 mt-2'>
									<Switch isSelected={useSimulation} size='sm' onValueChange={setUseSimulation}>
							...
							<ModalBody>
								<div className='flex flex-col items-center justify-center gap-4 py-4'>
									...
									<p className='text-center text-sm text-default-500'>Scan the QR code displayed on the driver&apos;s dashboard to confirm your ride.</p>
								</div>
							</ModalBody>
							<ModalFooter>
			...
			<Modal isOpen={isCancelConfirmOpen} placement='center' onOpenChange={onCancelConfirmOpenChange}>
				<ModalContent>
					{(onClose) => (
						<>
							<ModalHeader className='flex flex-col gap-1'>Cancel Trip Request?</ModalHeader>
							<ModalBody>
								<p className='text-sm text-default-600'>This will immediately cancel your current ride request. You will need to request a new ride if you still need transport.</p>
							</ModalBody>
							<ModalFooter>
								<Button variant='flat' onPress={onClose} isDisabled={isCancellingTrip}>
									Keep Request
								</Button>
								<Button color='danger' onPress={() => void handleConfirmCancel()} isLoading={isCancellingTrip}>
									{isCancellingTrip ? "Cancelling..." : "Yes, Cancel Trip"}
								</Button>
							</ModalFooter>
						</>
					)}
