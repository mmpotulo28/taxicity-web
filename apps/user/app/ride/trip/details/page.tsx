...
const TripDetails: React.FC = () => {
	...
	const handleRatingSubmit = async () => {
		...
		try {
			await apiPost(`/api/user/trips/${activeTrip.id}/rating`, { rating });
			...
			addToast({
				title: "Error",
				description: error instanceof ApiError ? error.message : "Could not submit rating.",
				color: "danger",
			});
	...
	return (
		<div className='h-full relative bg-default-100 overflow-hidden'>
			...
			<div className='absolute inset-0 z-0'>
				<MapView fullscreen showRoute={true} zIndex={0} />
			</div>

			{/* Content Overlay */}
			<div className='absolute inset-0 z-10 flex flex-col bg-background/60 backdrop-blur-md overflow-y-auto'>
				<motion.div animate={{ opacity: 1, y: 0 }} className='flex-1 flex flex-col p-4 max-w-md mx-auto w-full' initial={{ opacity: 0, y: 20 }} transition={{ duration: 0.4 }}>
					{/* Header */}
					<div className='py-8 text-center'>
						<motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 260, damping: 20, delay: 0.1 }} className='w-20 h-20 bg-success text-success-foreground rounded-full flex items-center justify-center mx-auto mb-4 shadow-xl shadow-success/20'>
								...
								<div className='flex items-center gap-3'>
									...
									<div className='flex-1'>
										...
										<p className='text-xs text-default-500'>
											{selectedTaxi.model} • {selectedTaxi.licensePlate}
										</p>
					...
					<Card className='shadow-sm border border-default-200 mb-6 bg-background/90 backdrop-blur-sm'>
						<CardBody className='p-4 text-center'>
							...
							<div className='flex justify-center gap-2 mb-4'>
								{[1, 2, 3, 4, 5].map((star) => (
									<button key={star} className='focus:outline-none transition-transform hover:scale-110' onClick={() => setRating(star)}>
										<Icon className={`text-3xl cursor-pointer transition-colors ${rating >= star ? "text-warning fill-warning" : "text-default-200"}`} icon='lucide:star' />
									</button>
								))}
							</div>
							<Button color='primary' variant='flat' className='w-full font-medium' isLoading={isSubmitting} onPress={handleRatingSubmit}>
								Submit Rating
							</Button>
						</CardBody>
					</Card>
					...
					<div className='mt-auto flex gap-3 pb-4'>
						<Button className='flex-1 font-medium bg-background/80 backdrop-blur-md' variant='bordered' startContent={<Icon icon='lucide:share-2' />} onPress={shareRide}>
							Share
						</Button>
						<Button className='flex-1 font-medium shadow-lg shadow-primary/20' color='primary' startContent={<Icon icon='lucide:home' />} onPress={handleHome}>
							Home
						</Button>
					</div>
				</motion.div>
