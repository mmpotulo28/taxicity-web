...
export function RankQueue({ taxiId, currentLocation, onJoin, onLeave, onStatusChange, onStartTrip }: RankQueueProps) {
 ...
 const fetchRanks = React.useCallback(async () => {
  ...
  try {
   const res = await fetch("/api/ranks?limit=50");
   const data = await res.json();

   // The API returns { ranks: [], pagination: {} }
   const ranksArray = Array.isArray(data) ? data : (data.ranks || []);
 ...
 const handleJoin = async () => {
  ...
  try {
   const res = await fetch("/api/driver/queue", {
    ...
    body: JSON.stringify({ rankId: selectedRank, taxiId }),
   });
 ...
 const handleLeave = async () => {
  ...
  setProcessing(true);
  try {
   const res = await fetch("/api/driver/queue", { method: "DELETE" });
 ...
 if (loading) return <div className="py-8 flex justify-center"><Spinner label="Checking queue status..." /></div>;
   ...
   <Card className="bg-primary-50 border-primary border shadow-md mb-6">
    <CardBody className="p-6">
     ...
     <div className="mt-6 flex gap-3">
      <Button
       className="flex-1 bg-white text-danger border border-danger/20 font-medium"
       variant="flat"
       onPress={handleLeave}
     ...
     {queueStatus.position === 1 && (
      <div className="mt-4 space-y-3">
       <motion.div
        ...
        className="p-3 bg-success-100 text-success-700 rounded-lg text-sm font-medium flex gap-2 items-center"
       >
       ...
       {queueStatus.rank?.sourceRoutes && queueStatus.rank.sourceRoutes.length > 0 && (
        <div className="bg-white p-4 rounded-lg border border-default-200 shadow-sm space-y-3">
         ...
         <Button
          ...
          onPress={() => onStartTrip?.(selectedRoute)}
 ...
 return (
  <div className="mb-6">
   ...
   <div className="flex gap-2">
    <Select
     placeholder="Select nearby rank to join..."
     className="flex-1"
    ...
    <Button
     color="primary"
     isDisabled={!selectedRank || !taxiId}
     isLoading={processing}
     onPress={handleJoin}
